//! Conservation tests for AgenticCodebase.
//!
...
use agentic_codebase::cache::LruCache;
use agentic_codebase::metrics::tokens::{Layer, TokenMetrics};
use agentic_codebase::query::budget::TokenBudget;
use agentic_codebase::query::intent::ExtractionIntent;
...
fn test_budget_enforcement() {
    ...
    // try_spend should fail when exceeding
    assert!(!budget.try_spend(80));
    assert_eq!(budget.remaining(), 70); // unchanged
}
...
fn test_conservation_score() {
    ...
    // Record a full retrieval: 100 tokens used
    metrics.record(Layer::Full, 100);

    // Record a cache hit: 0 tokens used
    metrics.record(Layer::Cache, 0);

    // Record savings: 400 tokens saved
    metrics.record_savings(400);
